...
source: "https://www.typescriptlang.org/docs/handbook/2/everyday-types.html#interfaces"
author:
published:
created: 2026-06-25
...
```markdown
ts// No type annotation needed -- 'myName' inferred as type 'string'
let myName = "Alice";
Try
```
...
```markdown
ts// Would be a runtime error if executed!
greet(42);
...
```markdown
tsfunction getFavoriteNumber(): number {
  return 26;
}
Try
```
...
```markdown
tsasync function getFavoriteNumber(): Promise<number> {
  return 26;
}
Try
```
...
```markdown
ts// The parameter's type annotation is an object type
...
'obj.last' is possibly 'undefined'.18048'obj.last' is possibly 'undefined'.  if (obj.last !== undefined) {
    // OK
    console.log(obj.last.toUpperCase());
  }
...
```markdown
tsfunction printId(id: number | string) {
...
> 
> ```markdown
> tsfunction printTextOrNumberOrBool(
>   textOrNumberOrBool:
>     | string
>     | number
>     | boolean
...
```markdown
tsfunction welcomePeople(x: string[] | string) {
  if (Array.isArray(x)) {
    // Here: 'x' is 'string[]'
...
```markdown
ts// Return type is inferred as number[] | string
...
```markdown
tstype UserInputSanitizedString = string;
...
| `Interface` | `Type` |
| --- | --- |
| Extending an interface  ```markdown interface Animal {   name: string; }  interface Bear extends Animal {   honey: boolean; }  const bear = getBear(); bear.name; bear.honey; ``` | Extending a type via intersections  ```markdown type Animal = {   name: string; }  type Bear = Animal & {    honey: boolean; }  const bear = getBear(); bear.name; bear.honey; ``` |
| Adding new fields to an existing interface  ```markdown interface Window {   title: string; }  interface Window {   ts: TypeScriptAPI; }  const src = 'const a = "Hello World"'; window.ts.transpileModule(src, {}); ``` | A type cannot be changed after being created  ```markdown type Window = {   title: string; }  type Window = {   ts: TypeScriptAPI; }   // Error: Duplicate identifier 'Window'. ``` |
...
```markdown
tsconst myCanvas = document.getElementById("main_canvas") as HTMLCanvasElement;
Try
```
...
```markdown
tsconst myCanvas = <HTMLCanvasElement>document.getElementById("main_canvas");
Try
```
...
```markdown
tslet changingString = "Hello World";
...
```markdown
tsfunction printText(s: string, alignment: "left" | "right" | "center") {
  // ...
}
...
```markdown
tsfunction compare(a: string, b: string): -1 | 0 | 1 {
...
```markdown
tsdeclare function handleRequest(url: string, method: "GET" | "POST"): void;
...
2. You can use `as const` to convert the entire object to be type literals:
	```markdown
	tsconst req = { url: "https://example.com", method: "GET" } as const;
	handleRequest(req.url, req.method);
	Try
	```
...
```markdown
tsfunction doSomething(x: string | null) {
  if (x === null) {
    // do nothing
  } else {
...
```markdown
tsfunction liveDangerously(x?: number | null) {
  // No error
...
```markdown
ts// Creating a bigint via the BigInt function
...
```markdown
tsconst firstName = Symbol("name");
const secondName = Symbol("name");
 
if (firstName === secondName) {
This comparison appears to be unintentional because the types 'typeof firstName' and 'typeof secondName' have no overlap.2367This comparison appears to be unintentional because the types 'typeof firstName' and 'typeof secondName' have no overlap.  // Can't ever happen
}
Try
```
...
- [[typescript-primitivos]]
- [[typescript-arrays]]
- [[typescript-tipo-any]]
- [[typescript-anotacoes-em-funcoes]]
